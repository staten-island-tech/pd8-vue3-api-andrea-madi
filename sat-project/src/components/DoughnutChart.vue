<template>
  <div class="container">
    <h1>% of SAT-test-takers per borough</h1>
...
export default {
  ...
  async mounted() {
    try {
      ...
      store.mData.push(store.mTotal.reduce((a, b) => a + b, 0))
      console.log(store.qTotal)
      store.qData.push(store.qTotal.reduce((a, b) => a + b, 0))
      console.log(store.bkTotal)
      store.bkData.push(store.bkTotal.reduce((a, b) => a + b, 0))
      ...
      this.chartData = {
        ...
        datasets: [
          {
            data: [store.mData, store.qData, store.bkData, store.siData, store.bxData],
            backgroundColor: [
              'rgb(255, 99, 132, .3)',
              'rgb(54, 162, 235, .3)',
              'rgb(290, 205, 86, .3)',
